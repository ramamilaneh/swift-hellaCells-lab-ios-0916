...
class HellaMasterTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
       self.numbers =  createNumbersArray()
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetails" {
            let destination = segue.destination as! HellaDetailsViewController
            destination.numberName = String(numbers[(tableView.indexPathForSelectedRow?.row)!])
        }
    }
